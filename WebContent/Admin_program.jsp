...
<body class="bg-light">
    <div class="p-3 text-dark sidebar-style">
        ...
        <ul class="nav nav-pills flex-column mb-auto">
            ...
            <li>
                <a href="AdminRedirectServlet?action=program" class="nav-link active">Program</a>
            </li>
            <li>
    ...
    <div class="content">
        ...
        <div class="col-md-7 col-lg-8 container" style="max-width: 900px;">
            <h4 class="mb-3">Maklumat Program</h4>
            <form action="AddProgramServlet" method="post" class="needs-validation" novalidate>
                <div class="row g-3">
                	<c:if test="${status != null}">
                		<c:choose>
							<c:when test="${status == 'New program added'}">
								<div class="alert alert-success text-center" role="alert"><c:out value="${status}"></c:out></div>
							</c:when>
							<c:when test="${status == 'Program Deleted'}">
								<div class="alert alert-success text-center" role="alert"><c:out value="${status}"></c:out></div>
							</c:when>
							<c:otherwise>
            ...
            <table id="table_id" class="display mb-4 pt-4 bg-light">
                ...
                <tbody>
					<c:forEach items="${programlist}" var="pl">
	                    <tr>
	                        <td><c:out value="${pl.progcode}" /></td>
	                        <td><c:out value="${pl.progname}" /></td>
	                        <td>
	                        	<c:if test="${session_level == 'Superadmin'}">
	                        		<button class="btn btn-danger"onclick="window.location.href='AdminRedirectServlet?action=deleteprogram&deleteid=<c:out value="${pl.progcode}" />'" >Delete</button>
	                        	</c:if>
	                        </td>
	                    </tr>
					</c:forEach>

	            ...
	            <table id="table_id" class="display mb-4 pt-4 bg-light">
	                ...
	                <tbody>
	                	<c:forEach items="${adminlist}" var="al">
	                		<c:if test="${session_id != al.adminid}">
	                			<tr>
			                        <td><c:out value="${al.adminid}" /></td>
			                        <td><c:out value="${al.adminname}" /></td>
			                        <td><c:out value="${al.adminphonenum}" /></td>
			                        <td><c:out value="${al.adminemail}" /></td>
			                        <td><c:out value="${al.superviseid}" /></td>
			                        <td><c:out value="${al.adminlevel}" /></td>
			                        <td>
			                        	<button class="btn btn-primary" onclick="window.location.href='AdminRedirectServlet?action=updateadmin&updateid=<c:out value="${al.adminid}" />'" >Update</button>
			                        	<button class="btn btn-danger" onclick="window.location.href='AdminRedirectServlet?action=deleteadmin&deleteid=<c:out value="${al.adminid}" />'" >Delete</button>
			                        </td>
			                    </tr>
	                		</c:if>
						</c:forEach>
